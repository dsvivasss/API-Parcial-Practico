...
describe('AirportService', () => {
  ...
  it('create should return a new airport', async () => {
    ...
    const newRecipy: AirportEntity = await service.create(airport);
    expect(newRecipy).not.toBeNull();

    const storedAirport: AirportEntity = await repository.findOne({ where: { id: newRecipy.id } })
    expect(storedAirport).not.toBeNull();
    expect(storedAirport.name).toEqual(newRecipy.name)
    expect(storedAirport.code).toEqual(newRecipy.code)
    expect(storedAirport.country).toEqual(newRecipy.country)
    expect(storedAirport.city).toEqual(newRecipy.city)
  });
